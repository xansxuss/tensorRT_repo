...
protected:
    ...
    virtual void releasResources();

private:
    size_t volume(const nvinfer1::Dims &d);
    size_t dataTypeSize(nvinfer1::DataType t);
};

class batchYoloInfer : public baseInfer
{
public:
    // batch inference class for YOLO
    // 這個類別是用來定義批次推論的介面
    // 它繼承自 baseInfer 並實作批次推論的邏輯
    explicit batchYoloInfer(const std::string &enginePath);
    ~batchYoloInfer();
    void init(const std::string &enginePath) override;
    void batchInference(BBox &Bbox);
    void batchInferenceGPU(BBox &Bbox);

protected:
    std::unique_ptr<batchYoloPreprocess> mPreprocess;
    // 這裡可以定義批次推論特有的成員變數和方法
    // 例如：批次大小、批次資料處理等
    // 這裡可以定義批次推論特有的成員變數和方法
    // 例如：批次大小、批次資料處理等
};
